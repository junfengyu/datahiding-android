...
public class DownloadPicture extends AsyncTask<Void, Long, Boolean> {
    ...
    protected Boolean doInBackground(Void... params) {
    	   try {
               ...
               int index = thumbs.size();//(int)(Math.random() * thumbs.size());
               Entry ent = thumbs.get(index-1);
               String path = ent.path;
               mFileLen = ent.bytes;

               File sdCard = Environment.getExternalStorageDirectory();

               File dir = new File (sdCard.getAbsolutePath() + "/receiver");
               if(!dir.exists()){
                   dir.mkdirs();
               }
              // File outFile = new File(dir, IMAGE_FILE_NAME);
               String cachePath = dir.toString() + "/" + IMAGE_FILE_NAME;
               try {
               ...
               mApi.getFileStream(path,null).copyStreamToOutput(mFos,null);
               if (mCanceled) {
                   return false;
               }
